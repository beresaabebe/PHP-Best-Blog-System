        ...
        <div class="col-xs-12">
          <div class="box">
            <div class="box-header">
              <h3 class="box-title"><?php echo _l_number_of_unapproved_comments; ?></h3>
            </div>
            <!-- /.box-header -->
...
<tr title="This comment is created by: <?php echo ucfirst($author_firstname).' '.ucfirst($author_lastname); ?> on: <?php echo date('M,Y',strtotime(formatDate($comment_date))); ?>">
  ...
  <td style="color:red"><?php if ($comment_status =='0') {
    echo lcfirst(_l_unapproved);
  } ?></td>
  <td><a class="btn btn-success btn-xs" href="index.php?source=<?php echo urlencode('lists of unapproved comments');?>&approve=<?php echo $comment_id; ?>" title="Do you want to approve this comment?"><i class="fa fa-check-square-o"></i> <?php echo ucfirst(_l_approve); ?></a></td>
...
<script type="text/javascript">
  alert('This comment is already approved!');
    ...
    $execute = mysqli_query($connection,$query);
    confirmQuery($execute);
    $encode =urlencode('lists of unapproved comments');
